...
Every hospital is required by CMS (45 CFR Part 180) to publish a machine-readable file (MRF) with their chargemaster. The files are 100 KB to 3 GB each, in five different formats (CMS v3 JSON, tall CSV, wide CSV, XLSX, ZIP), and several large systems sit behind Akamai TLS bot walls. Nobody downloads them at scale. We did.

## The headline

> A total knee replacement at one US hospital is **$9,291**. At another it is **$83,902**. Same CPT 27447, same procedure description, 9x spread. Doxorubicin (HCPCS J9000, per 10 mg of the chemo drug) goes from **$44 to $3,376**, a 78x spread on the same drug at the same unit. Palonosetron (HCPCS J2469, anti-nausea per 25 mcg): **$35 to $5,499**, 159x.
...
| Docker image | **`python:3.12`** (Burla auto-installs missing packages) |
| LLMs used | **zero** |
...
pipeline.py           Burla worker: download streaming + parse + write jsonl
...
run_reduce_burla.py   run reduce + analysis on a single Burla worker
...
python scale.py                  # map across all hospitals on Burla
...
## How the parallel dispatch looks
...
`func_cpu=1` and `func_ram=2` so each MRF parse only takes one CPU slot. On a 13 by 80-CPU cluster that is 1,040 calls running concurrently. Burla queues the rest. `print` from inside `parse_hospital_mrf` lands in the dashboard Jobs tab in real time.

Workers write their parsed observations to `/workspace/shared/hpt/observations/<hospital_id>.jsonl`, which is GCSFuse-backed and visible on every container in the cluster. The reduce step is a single 8 vCPU container that walks the shared tree, deduplicates `(hospital, code)` variants into one representative observation, and emits the static JSON the React site reads.
